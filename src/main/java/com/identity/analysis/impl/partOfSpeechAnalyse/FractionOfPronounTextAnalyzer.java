 ...
 */
public class FractionOfPronounTextAnalyzer extends FractionOfPOSTextAnalyzer<FractionOfPronoun> {
    @Override
    public Measure getMeasure(List<String> texts) {
        ...
        return new FractionOfPronoun(TextAnalyzer.expectedValue(texts, res));
    }

    @Override
    public double analyse(String text) {
        return analyse(text, PartOfSpeech.PRON) + analyse(text, PartOfSpeech.PN);
    }
}
